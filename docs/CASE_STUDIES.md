...
| 2026-07-21 | Defensive Text-Wrapping & Database Reset Suite: applied `break-words break-all` across confirmation modals, history table, and Step 3 cards to eliminate layout overflow from long unbroken strings (e.g. `thisisatest...`). Added input length limits (`maxLength` on titles, tags, notes), added `reset_analysis_history()` and `POST /api/reset-db` endpoint, and wiped local SQLite database resetting auto-increment sequence back to ID 1 for clean testing. 43/43 tests passing 100%, Next.js build compiled in 1605ms. | AI Coding Agent |
