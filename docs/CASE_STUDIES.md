...
| 2026-07-19 | Create Process Sheet UI Redesign: replaced emoji-heavy stepper wizard with 2-phase clean studio layout; live DINOv2 scan triggered on image upload (not on submit); negative cosine similarity values clamped to 0.00% in all layers (db.py, app.py, page.tsx); duplicate pattern submission locked when similarity >= 90%; navigation buttons anchored correctly to bottom corners. | AI Coding Agent |
| 2026-07-20 | Top-K Similarity Catalog Inspector: replaced single "Highest Catalog Match" card with scrollable Top 5 Matched Projects list (inline, no page navigation); fixed garment_type display reading nested `project_details.garment_type` path in db.py; backend limit raised from 3 to 5 top candidates; redundant duplicate percentage in header removed. 34/34 tests passing. | AI Coding Agent |
| 2026-07-21 | Phase 22 Advanced Pre-Production Engineering & Line Balancing Suite: implemented `derive_work_aids_from_sequence()` (Folders, Binders, Edge Guides, Acrylic Templates, Ultrasonic Jigs) and `calculate_line_balancing()` (Takt Time $0.96\text{m/unit}$, JUKI machine unit counts at 85% efficiency, bottleneck step identification). Added UI cards to Step 3 in `page.tsx` and 3 unit tests in `test_backend_contract.py` (40/40 tests PASS 100%). | AI Coding Agent |
| 2026-07-21 | Interactive Master Spec Reuse & Layout Locking Suite: replaced rigid duplicate block (Red/Yellow mismatch + browser alert popup) with an interactive Cobalt Blue Catalog Reuse Prompt Card (`REUSE MASTER SPEC` vs `CREATE NEW VARIANT`). Added `is_reuse_master` API flag to recalculate batch runs (e.g. 500 pcs vs 1000 pcs) without creating redundant DB rows. Fixed table column layout shift via `table-fixed` CSS with explicit percentage column widths (`12%`, `40%`, `10%`, `38%`), updated SMV unit text to explicit `1.5 mins`, and rendered light-mode 2D Pattern CAD Blueprint SVGs. 41/41 unit tests passing, Next.js build compiled in 1669ms. | AI Coding Agent |
| 2026-07-21 | Dynamic Project Tag Management & Multi-Criteria History Search/Filter Suite: replaced legacy Batik motif classifier with GitHub/LinkedIn-style Tag Picker (`TagSelector` component + `GET /api/tags`), optional Designer & Pattern Notes textarea, and a multi-criteria search & status filter toolbar in Active Projects & History with live count badges (`All Statuses`, `Approved`, `Duplicate Locked`). 42/42 tests passing 100%, Next.js build compiled in 1674ms. | AI Coding Agent |
| 2026-07-21 | KB 2-Column Responsive Grid, Tag Enter Key Fix & Step 3 Tags/Notes UI Upgrade: converted Manufacturing Knowledge Base to a 2-column responsive grid (`grid grid-cols-1 md:grid-cols-2 gap-6`), added `onKeyDown` Enter key intercept to `TagSelector` search input to prevent accidental form submission, implemented Step 2 Compilation Review Confirmation Modal (`showProcessSheetConfirmModal`), replaced legacy `Classified Motif` text on Step 3 Pattern Metadata with `Project Tags:`, and rendered a dedicated `Designer & Engineering Notes` card on Step 3 main process sheet view. 42/42 tests passing 100%, Next.js build compiled in 1732ms. | AI Coding Agent |
| 2026-07-21 | Tag Leftover Cleanup & Dynamic DB Tag Sync: updated `TagSelector` to sync available tags strictly from database records (`GET /api/tags` and `analysisHistory`). Removed uncommitted custom tag mutation from draft form state so removing uncommitted tag pills with "✕" leaves zero leftovers in the dropdown menu. 42/42 tests passing 100%, Next.js build compiled in 1606ms. | AI Coding Agent |
| 2026-07-21 | Full System Hardening & Security Vulnerability Audit: added strict upload format validation rejecting non-image payloads (MP4, EXE, BIN) with 400 Bad Request, path traversal sanitization on `model_name` input (`os.path.basename`), input length truncation on `/api/generate-sheet`, and parameterized SQL query auditing. Added 2 regression tests (`test_non_image_upload_rejection_mp4` and `test_sql_injection_payload_safety`). 45/45 tests PASS (100%), Next.js build compiled in 1578ms. | AI Coding Agent |
| 2026-07-21 | Smart Action Menu Positioning & Container Overflow Fix: updated history table container to `overflow-visible` and applied dynamic direction calculation (`bottom-full mb-1` for bottom 2 table rows) on `activeMenuId` action menu to eliminate UI clipping/truncation. 45/45 tests PASS (100%), Next.js build compiled in 1492ms. | AI Coding Agent |
